...
vsd_ip = '147.75.69.51'
api_url = "https://147.75.69.51:8443"
username = "csproot"
password = "csproot"
enterprise = "csp"
...
VSC1_IP = "192.168.0.31"
infrVsc2Name = 'vsc2.sdn40r8.lab'
VSC2_IP = "192.168.0.32"
infrVsc3Name = 'vsc3.sdn40r8.lab'
VSC3_IP = "192.168.0.33"
infrVsc4Name = 'vsc4.sdn40r8.lab'
VSC4_IP = "192.168.0.34"
...
nsgTemplate3Name = 'NSGV_2WAN_4LAN'
nsgTemplate4Name = 'NSGE_2WAN_4LAN'

# Create NSG instances from the NSG templates
NSGName1="head1"
NSGName2="head2"
NSGName3="border"
NSGName4="nsg-branch1-1"
NSGName5="nsg-branch1-2"
NSGName6="nsg-branch2-1"
NSGName7="nsg-branch2-2"
...
######################################################
# Create NSG-V template (2 network, 4 access) 
######################################################
def func_create_nsg_2w_4l_template(csprootSession,nsgTemplateName,
    ...
    if not nsgateway_nsg:
        print 'INFO: Creating NSG-V Template (2 network, 4 access)',
        ...
        print ('INFO: NSG-V Template (2 network, 4 access) is already created')
...
######################################################
def func_create_nsge_2w_4l_template(csprootSession,nsgTemplateName,
                                infrastructure_gw_profile,infra_access_profile,
                                infrastructure_vsc_profile1,infrastructure_vsc_profile2):
    nsgateway_nsg = csprootSession.ns_gateway_templates.get_first(filter="name == '%s'" % nsgTemplateName)
    if not nsgateway_nsg:
...
# Creating subnets                        #
###########################################
def func_create_subnet(csprootSession,zone,subnetName,zoneDescription,SubnetIpAddr,SubnetPrefix,SubnetGateway,dhcpminRange,dhcpmaxRange):
    new_subnet = zone.subnets.get_first(filter="name == '%s'" % subnetName)
    if not new_subnet:
        print 'INFO: Creating A Subnet', subnetName
        new_subnet=vspk.NUSubnet(name=subnetName,description=zoneDescription,address=SubnetIpAddr,netmask=SubnetPrefix,gateway=SubnetGateway)
...
def main():
    ...
    ######################################################
    # create 4 NSG templates (1WAN_1LAN, 2WAN_2LAN, 2WAN_4LAN)
    ######################################################
    ...
    nsge_template_2w_4l = func_create_nsge_2w_4l_template(csprootSession,nsgTemplate4Name,infrastructure_gw_profile,
        infra_access_profile,infrastructure_vsc_profile1,infrastructure_vsc_profile2)
    ...
    dc_subnet1_zone1 = func_create_subnet(csprootSession,new_dc_zone1,dcsubnet1Name,dczone1Description,dcsubnet1IpAddr,
                                           dcsubnet1Prefix,dcsubnet1Gateway,dcsubnet1DhcpMinRange,dcsubnet1DhcpMaxRange)
    dc_subnet2_zone1 = func_create_subnet(csprootSession,new_dc_zone1,dcsubnet2Name,dczone1Description,dcsubnet2IpAddr,
                                           dcsubnet2Prefix,dcsubnet2Gateway,dcsubnet2DhcpMinRange,dcsubnet2DhcpMaxRange)
    dc_subnet3_zone1 = func_create_subnet(csprootSession,new_dc_zone1,dcsubnet3Name,dczone1Description,dcsubnet3IpAddr,
                                           dcsubnet3Prefix,dcsubnet3Gateway,dcsubnet3DhcpMinRange,dcsubnet3DhcpMaxRange)
    dc_subnet4_zone1 = func_create_subnet(csprootSession,new_dc_zone1,dcsubnet4Name,dczone1Description,dcsubnet4IpAddr,
                                           dcsubnet4Prefix,dcsubnet4Gateway,dcsubnet4DhcpMinRange,dcsubnet4DhcpMaxRange)

    ############################################# 
    # create 3 New NSGs4Instances; one from each template
    ############################################# 
    ...
    func_create_nsg(new_enterprise,nsgtemplate_2w_4l,NSGName4)
    func_create_nsg(new_enterprise,nsgtemplate_2w_4l,NSGName5)
    func_create_nsg(new_enterprise,nsgtemplate_2w_4l,NSGName6)
    func_create_nsg(new_enterprise,nsgtemplate_2w_4l,NSGName7)
